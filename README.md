...
* `origin`: Configures the **Access-Control-Allow-Origin** CORS header. Possible values:
  ...
  - `RegExp` - set `origin` to a regular expression pattern which will be used to test the request origin. If it's a match, the request origin will be reflected. Prefer anchored patterns that match a full host boundary, for example `/^https:\/\/([a-z0-9-]+\.)*example\.com$/`, so that `https://example.com` and `https://api.example.com` match but `https://evil-example.com` does not. Avoid unanchored suffix patterns like `/example\.com$/`, which also match attacker-controlled hosts such as `https://evil-example.com`.
  - `Array` - set `origin` to an array of valid origins. Each origin can be a `String` or a `RegExp`. For example `["http://example1.com", /^https:\/\/([a-z0-9-]+\.)*example2\.com$/]` will accept any request from "http://example1.com" or from `example2.com` and its subdomains (but not `evil-example2.com`).
...
* `credentials`: Configures the **Access-Control-Allow-Credentials** CORS header. Set to `true` to pass the header, otherwise it is omitted. **Warning:** combining `credentials: true` with `origin: true` reflects *any* request `Origin` and allows that origin to read credentialed responses (cookies, HTTP authentication). Prefer an explicit allowlist of trusted origins when credentials are enabled.
