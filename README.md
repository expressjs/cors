...
* `origin`: Configures the **Access-Control-Allow-Origin** CORS header. Possible values:
  ...
  - `RegExp` - set `origin` to a regular expression pattern which will be used to test the request origin. If it's a match, the request origin will be reflected. For example the pattern `/example\.com$/` will reflect any request that is coming from an origin ending with "example.com". **Warning: The Regexp test fails sometimes when you use the global flag (`/g`)**. See [this issue](https://github.com/expressjs/cors/issues/220) for more information.
